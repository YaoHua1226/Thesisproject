...
import matplotlib
from Generation_method import SolarPVModel

# 设置 matplotlib 字体及防止负号显示为方块
matplotlib.rcParams['font.sans-serif'] = ['SimHei']
matplotlib.rcParams['axes.unicode_minus'] = False

# 读取并合并 2021-2025年 (1-4月) 的 NASA POWER 数据
df_list = []
...
# 输出 CSV
for df, fn in [(res_2024_mixed, 'result_2024_mixed.csv'),
               (res_2024_pred, 'result_2024_predicted.csv'),
               (res_2025_pred, 'result_2025_predicted.csv')]:
    df.rename(columns={'datetime': 'Datetime'}, inplace=True)
    df.to_csv(fn, index=False)

# 绘制图表：保持英文字体和风格
# 2024 年 Temperature 对比 (Actual vs Predicted, Full Year)
...
# 2024 年 GHI 对比 (Actual vs Predicted, Full Year)
plt.figure(figsize=(8,5))
...
# 2025 年温度对比 (实际 Jan–Apr + 预测 May–Dec vs 全年预测) - 按小时精度
# 2025 Temperature Comparison (Jan–Apr Actual + May–Dec Predicted vs Full-Year Predicted) – hourly resolution
...
# 扩展预测至 2026–2033（生成预测数据、发电量计算及图表）
for year in range(2026, 2034):
    # 构建全年预测数据集
    time_index_year = pd.date_range(f"{year}-01-01 00:00:00", f"{year}-12-31 23:00:00", freq="h")
    df_year = pd.DataFrame({'Datetime': time_index_year})
    # 提取日期字段
    for col in ['Month', 'Day', 'Hour']:
        df_year[col] = getattr(df_year['Datetime'].dt, col.lower())
    # 合并典型日，获取 GHI、Temp 预测值
    df_year = df_year.merge(typical, on=['Month', 'Day', 'Hour'], how='left')
    df_year.rename(columns={'GHI': 'GHI_pred', 'Temp': 'Temp_pred'}, inplace=True)
    df_year_predicted = df_year[['Datetime', 'GHI_pred', 'Temp_pred']].copy()
    df_year_predicted.rename(columns={'GHI_pred': 'GHI', 'Temp_pred': 'Temp'}, inplace=True)
    # 使用 SolarPVModel 计算发电功率，输出全年小时数据 CSV
    res_year_pred = model.compute_power(df_year_predicted.rename(columns={'Datetime': 'datetime'})[['datetime', 'GHI', 'Temp']])
    res_year_pred.rename(columns={'datetime': 'Datetime'}, inplace=True)
    res_year_pred.to_csv(f'result_{year}_predicted.csv', index=False)
    
    # 绘制全年 GHI 预测曲线
    plt.figure(figsize=(8,5))
    plt.plot(df_year_predicted['Datetime'], df_year_predicted['GHI'], label='Predicted')
    plt.title(f'{year} GHI Prediction (Full Year)')
    plt.xlabel('Date')
    plt.ylabel(r'GHI (W/m$^2$)')
    plt.legend()
    plt.savefig(f'ghi_{year}_predicted_en.png')
    plt.show()
    # 绘制全年 Temperature 预测曲线
    plt.figure(figsize=(8,5))
    plt.plot(df_year_predicted['Datetime'], df_year_predicted['Temp'], label='Predicted')
    plt.title(f'{year} Temperature Prediction (Full Year)')
    plt.xlabel('Date')
    plt.ylabel('Temperature (°C)')
    plt.legend()
    plt.savefig(f'temp_{year}_predicted_en.png')
    plt.show()
